 ...
 */
public class ScriptPreferencePage extends PreferencePage implements IWorkbenchPreferencePage, IPreferenceConstants {
	...
	private Text fStartScriptTextField;
	...
	private String[] DOUBLE_CLICK_BEHAVIOURS = { Messages.ScriptPreferencePage_4, Messages.ScriptPreferencePage_5,
	...
	private String[] JS_VERSIONS = { Messages.ScriptPreferencePage_10, Messages.ScriptPreferencePage_11,
	...
	protected Control createContents(Composite parent) {
		...
		// Script path
		label = new Label(settingsGroup, SWT.NULL);
		label.setText("On start");

		fStartScriptTextField = UIUtils.createSingleTextControl(settingsGroup, SWT.BORDER, false);
		fStartScriptTextField.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));

		Button folderButton2 = new Button(settingsGroup, SWT.PUSH);
		folderButton2.setText(Messages.ScriptPreferencePage_2);
		folderButton2.addSelectionListener(widgetSelectedAdapter(event -> {
			String path = chooseEditor();
			if (path != null) {
				fStartScriptTextField.setText(path);
			}
		}));
	...
	private void setValues() {
		...
		fStartScriptTextField.setText(getPreferenceStore().getString(PREFS_START_SCRIPT));
	...
	public boolean performOk() {
		...
		getPreferenceStore().setValue(PREFS_START_SCRIPT, fStartScriptTextField.getText());
		...
		getPreferenceStore().setValue(PREFS_CONSOLE_FONT,
				fDefaultConsoleFontData.equals(fConsoleFontData) ? "" : fConsoleFontData.toString()); //$NON-NLS-1$
